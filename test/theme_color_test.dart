...
void main() {
  setUp(() {
    ...
    AppColors.primary = AppThemeColor.red.color;
    AppColors.primaryButton = AppThemeColor.red.buttonColor;
  });
  ...
  test('derived shades follow primary and stay in the brand hue', () {
    ...
    expect(
      l(AppColors.primaryTint),
      greaterThan(l(AppColors.primaryTintStrong)),
    );
    expect(
      l(AppColors.primaryTintStrong),
      greaterThan(l(AppColors.primarySoft)),
    );
  ...
  testWidgets('changing the theme color repaints a live widget tree', (
    tester,
  ) async {
    ...
    Color painted(String key) =>
        tester.widget<ColoredBox>(find.byKey(Key(key))).color;

    expect(painted('direct'), AppThemeColor.red.color);
    expect(
      painted('themed'),
      AppThemeColor.red.color,
      reason: 'ThemeData must carry the brand color',
    );
    final brandTint = painted('tint');

    await capturedRef
        .read(themeColorProvider.notifier)
        .setColor(AppThemeColor.green);
    await tester.pumpAndSettle();

    expect(
      painted('direct'),
      AppThemeColor.green.color,
      reason: 'direct AppColors read must update',
    );
    expect(
      painted('themed'),
      AppThemeColor.green.color,
      reason: 'Theme.of() consumers must update',
    );
    expect(
      painted('tint'),
      isNot(brandTint),
      reason: 'derived tints must update too',
    );
  });
}
