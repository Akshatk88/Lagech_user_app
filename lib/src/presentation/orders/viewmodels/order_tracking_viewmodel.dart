...
class OrderTrackingViewModel extends Notifier<OrderTrackingState> {
  ...
  Future<void> start(String orderId) async {
    String cleanId = orderId.replaceFirst('#', '').trim();
    final active = ref.read(activeOrderViewModelProvider).activeOrder;
    if (cleanId.isEmpty || cleanId == 'null' || cleanId == 'undefined') {
      if (active != null && active.id.isNotEmpty) {
        cleanId = active.id;
      }
    }
    if (_orderId == cleanId && (_rtdbSub != null || _socketSub != null)) return;
    _teardown();
    _orderId = cleanId;
    ...
    if (active != null && (cleanId.isEmpty || active.id == cleanId || active.orderNumber == cleanId || cleanId.contains(active.orderNumber))) {
      state = OrderTrackingState(
        order: active,
        isLoading: false,
        restaurantLat: active.restaurantLat,
        restaurantLng: active.restaurantLng,
        customerLat: active.dropLat,
        customerLng: active.dropLng,
        riderLat: active.riderLat,
        riderLng: active.riderLng,
        hasRiderFix: active.riderLat != null,
      );
    } else {
  ...
  Future<void> refresh() async {
    try {
      var idToFetch = _orderId.replaceFirst('#', '').trim();
      if (idToFetch.isEmpty || idToFetch == 'null' || idToFetch == 'undefined') {
        final active = ref.read(activeOrderViewModelProvider).activeOrder;
        if (active != null && active.id.isNotEmpty) {
          idToFetch = active.id;
          _orderId = idToFetch;
      ...
      final order = await ref.read(orderRemoteDataSourceProvider).getOrderModel(idToFetch);
