...
class ActiveOrderViewModel extends Notifier<ActiveOrderState> {
  ...
  Future<void> fetchActiveOrder({bool isRefresh = false}) async {
    try {
      ...
      if (activeList.isNotEmpty) {
        ...
        ref.read(ordersViewModelProvider.notifier).prependOrderDirectly(latestActive);

        _joinRoom(latestActive.id);
        _startPolling();
      } else {
        final existing = state.activeOrder;
        final serverMatch = existing != null
            ? result.orders.cast<OrderModel?>().firstWhere(
                (o) => o != null && (o.id == existing.id || o.orderNumber == existing.orderNumber),
                orElse: () => null,
              )
            : null;

        if (serverMatch != null && !serverMatch.isActive) {
          if (kDebugMode) {
            debugPrint('[HOME] Active order completed/cancelled on server. Clearing floating widget.');
          }
          ...
          if (kDebugMode) {
            debugPrint('[HOME] Preserving local active order ${existing.id}');
          }
          _joinRoom(existing.id);
          _startPolling();
        } else {
  ...
  void setActiveOrderDirectly(OrderModel newOrder) {
    if (!newOrder.isActive) return;
    state = state.copyWith(activeOrder: newOrder, isLoading: false);
    _joinRoom(newOrder.id);
    _startPolling();
  }
